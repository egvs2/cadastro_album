...
def tela_cadastro():
    global root_cadastro, nome_album, ano, nome_criador, lancamento, mensagem
    root_cadastro = Tk()
    ...
    command=lambda: ops.registro(nome_album, ano, nome_criador, lancamento, mensagem)).place(x=140, y=240)   
...
def tela_nome():
    ...
    command=lambda: ops.busca_nome(nome_artista, mensagem)).pack()
...
def tela_ano():
    ...
    command=lambda: ops.busca_ano(lista_albuns, valor_rb, cb)).pack()
